...
export const reviewsApi = {
  list: (params: ListReviewsParams = {}) =>
    api.get<ReviewListResponse>('/v1/reviews', { params }).then((r) => r.data),

  findOne: (id: string) => api.get<Review>(`/v1/reviews/${id}`).then((r) => r.data),

  create: (payload: ReviewPayload) => api.post<Review>('/v1/reviews', payload).then((r) => r.data),

  update: (id: string, payload: Partial<ReviewPayload>) =>
    api.patch<Review>(`/v1/reviews/${id}`, payload).then((r) => r.data),
  ...
  setApproved: (id: string, approved: boolean) =>
    api.patch<Review>(`/v1/reviews/${id}/approved`, { approved }).then((r) => r.data),

  remove: (id: string) => api.delete<{ success: boolean }>(`/v1/reviews/${id}`).then((r) => r.data),
};
